...
{ 
  ...
  G4int sipm_placed = 0;                                                                              
  G4double max_x = PS_x / 2.0 - border_gap - SD_width / 2.0;      
  G4double min_x = -max_x;                                                                       
  G4double max_y = PS_y / 2.0 - border_gap - SD_width / 2.0;                                                 
  G4double min_y = -max_y;                                                                      
  G4double x_spacing_max = max_x * 2 / (total_SiPM / num_rows - 1); 
  G4double y_spacing_max = max_y * 2 / (num_rows - 1);  
  for (G4int row = 0; row < num_rows; row++) {                              
    for (G4int col = 0; col < total_SiPM / num_rows; col++) {                              
      G4double x_pos = min_x + col * x_spacing_max;                                    
      G4double y_pos = min_y + row * y_spacing_max;                                      
      // SiPM의 고유 이름 생성
      std::ostringstream sipmName;
      sipmName << "SiPM_" << sipm_placed;
